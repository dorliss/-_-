...
3. Придумывем имя пользователя, пароль, имя хоста, и доменное имя (на ход лабораторной это почти никак не повлияет, но на будущее пригодится). Затем нажимаем "Далее"
   ![](pics/03.png)
4. Теперь выбираем оборудование. Оперативной памяти берем минимум 4 ГБ или же 4096 МБ, иначе ничего гружиться не будет. И количество процессоров можно оставить 1, но я взяла 3 (Хотя по личному опыту скажу, что роли это вообще никакой не играет, все равно виснет). Затем нажимаем "Далее". Создаем новым выртуальный жестний диск. Нужно взять не менее 40 ГБ (иначе ничего не влезет). И снова далее.
   ![](pics/04а.png)
   ![](pics/05.png)
6.  Проверяем настройки и нажимаем "Готово". Машина создана.
   ![](pics/06.png)
   ![](pics/07.png)
